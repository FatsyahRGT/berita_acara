...
<form action="" method="post">
    <div class="mb-3 row">
        <div class="col-sm-2">
            <label for="level_id" class="form-label">Nama Level</label>
        </div>
        <div class="col-sm-6">
            <select name="level_id" id="level_id" class="form-control">
                <option value="">Pilih Level</option>
                <?php while ($rowLevel = mysqli_fetch_assoc($queryLevels)) : ?>
                    <option value="<?= htmlspecialchars($rowLevel['id']) ?>"
                        <?= isset($rowEdit) && $rowEdit['level_id'] == $rowLevel['id'] ? 'selected' : '' ?>>
                        <?= htmlspecialchars($rowLevel['level_name']) ?>
                    </option>
                <?php endwhile; ?>
            </select>
        </div>
